...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    printf("===zheli zhixinglema \n");
    printf("=====You mean can not find valid images?\n");
    list *plist = get_paths(valid_images);
    printf("========%s\n", plist[0]);
    char **paths = (char **)list_to_array(plist);
    printf("=====How about this?Did u find that?\n");
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(fgets(line_image_path, sizeof(line_image_path), testfp)){
       ...
       //    if (filename) break;
        ...
        strcpy(temp_image_name, image_file_name);
        strcat(temp_image_name, ".png");
        printf("===>Temp Image Name %s \n", temp_image_name);
        for(k=0;k<l.w*l.h*l.n;++k){
           box b =  boxes[k];
